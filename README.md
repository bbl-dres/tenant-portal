...
- **`npm run verify:all`** — the Playwright checks in `scripts/verify/`, individually runnable: `verify:mobile-nav` (burger-menu behaviour), `verify:mobile-layouts` (horizontal-overflow / control-sizing at phone widths, incl. the reviewer queue), `verify:header-chrome` (split logo, breadcrumb, burger placement across breakpoints), `verify:property-images` (all imagery same-origin, nothing broken), `verify:media-viewer` (document preview: prev/next navigation, single-scroll model with sticky header, share popover + `?doc=` deep link; property image gallery: open, upload, navigate, delete, header-badge sync), `verify:a11y` (responsive accessibility sweep: landmarks, names, headings, focus management). JSON reports and screenshots land in `verify_out/` (gitignored, uploaded as a CI artifact).
