...
> **As of:** 2026-05-18 · **Methodology:** HERMES (Swiss federal project management standard) · **Prioritization:** MoSCoW (Must / Should / Could / Won't)
>
...
- **Functional concept (Fachkonzept):** [SUPERB_Fachkonzept_ERP_IMMO_Mieterportal NEW V05](BBL%20Requierements/intermediate/SUPERB_Fachkonzept_ERP_IMMO_Mieterportal%20NEW%20V05.md) — the authoritative requirements document for the pilot, edited by Holger Ludwig (BBL), Product Owner Heinz Ryter, as of 2025-11-10.
...
| FUNC-LP-008 | Responsive design that works on mobile devices, at least for the publicly accessible (non-authenticated) parts of the portal | **Should** | As a federal employee using a phone, I want to read public information on the go, so that I am not tied to a desktop machine. | Workshop WS1 | Use Case Landing Page V2.0 | Mobile maturity for authenticated areas to be assessed separately because of device-compliance rules. |
| FUNC-LP-009 | Every piece of content on the portal carries: (a) its source, (b) the responsible contact person, (c) the date when it was last updated | **Must** | As a tenant, I want to know who is responsible for each piece of information and when it was last reviewed, so that I can trust what I am reading. | Workshop WS1 | Use Case Landing Page V2.0 | These are "defined quality requirements" — acceptance criterion per content item. |
...
| FUNC-AU-019 | For "major applications" (Grossantrag — typically large construction projects over a CHF threshold), the form requires twelve mandatory free-text fields covering: short description, deficiencies in the current situation, legal basis, operational goals, alternatives that were considered, cost-benefit justification, target state, planning dependencies, schedule, cost expectation, full-time equivalents (FTEs), and workstation count | **Must** | As an applicant, I want one structured form that captures everything a major application needs, so that the reviewing General Secretariat (GS) is ready to assess without round-trips. | BBL PFM | Fachkonzept NEW V05 §4.1.1.3.4 (rows 4.1–4.12) | Twelve fields mapped to specific SAP ePPM tabs (see FUNC-PFM-002). |
...
| NFA-UX-003 | Responsive design for mobile devices, at least on the publicly accessible parts | **Should** | As a mobile user, I want to read public content on the move, so that I am not tied to a desktop. | Workshop WS1 | Use Case Landing Page V2.0 | Mobile maturity for authenticated areas to be evaluated separately. |
| NFA-UX-004 | Every content item shows its source, contact person, and "last updated" date | **Must** | As a tenant, I want to know where information comes from and when it was last reviewed, so that I can rely on it. | Workshop WS1; BBL Campus | Use Case Landing Page V2.0 | Same intent as FUNC-LP-009. |
...
| ID | Requirement | Priority | User story | Requested by | Source | Note |
|---|---|---|---|---|---|---|
| NFA-CD-001 | Use the **Swiss Federal Design System** (HTML structures, CSS assets, design tokens, components) to build the portal UI | **Must** | As a tenant, I want the portal to look and feel like every other federal site, so that I recognise it as official and do not need to relearn interaction patterns. | Federal Chancellery | [github.com/swiss/designsystem](https://github.com/swiss/designsystem) | Internal tool — minimum implementation: typographic tokens, colour palette, base component set, Swiss coat of arms / federal brand. Use the Nuxt/Web-Components integration if compatible with the chosen frontend stack. |
...
| NFA-CD-003 | Localise the portal into the three Swiss official languages: **German (DE)**, **French (FR)**, **Italian (IT)** | **Should** | As a French- or Italian-speaking federal tenant, I want the portal in my official language, so that communication and legal certainty are guaranteed. | Federal Department of Foreign Affairs (FDFA); all FR/IT-speaking VEs; Compliance (federal Languages Act) | Implicit in federal language law; Use Case Landing Page §UX requirements | English only if a concrete need arises (e.g. FDFA staff posted abroad). Replaces the earlier UX-language requirement. |
...
| OP-4 | "BBL PFM as requester (as of 2025-11-04)" appears throughout; specific names (Holger Ludwig and others) are only visible in the solution-architecture annex | Fachkonzept NEW V05 §4.1.1.3 | List the PFM representatives explicitly and tidy the "Requested by" column. |
...
| OP-9 | The explicit list of "responsible BBL roles" per VE (looked up by FUNC-AU-001) is missing | FUNC-AU-001; Use Case Bedarf | Compile with BBL PFM (Holger Ludwig). |
