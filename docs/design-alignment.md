...
| Home quick-services band | S: grey icon tiles (portal invention — verified absent from CD); T: white arrow cards; CTAs differ (S: CD `.section__action` bare button · T: bold primary link with hover slide) | **D41**: white `card--quick` + `.card-grid` in both; CTA = CD `.section__action` anatomy in both |
| Prototype banner text↔action | T: 16 px gap + `lg:ml-6`, whole lead sentence bold; S: button flush under the text, single word bold | **D42**: T's spacing + emphasis in both |
| Profile page | T: centred column with identity/notifications/language cards (+ demo role rows); S: none — the header name was a dead label | **D43**: page ported to S (name → `#/profile` link in both); T deletes the role rows (demo machinery, not identity); S centres the content in the same narrow column |
| Home greeting | T: `greeting-strip` (time-of-day + open-case count as a link); S: none | **D44**: ported to S above the case-preview table, same values |
| Hero title top margin | none in either | **D45**: `hero__title` margin-top 2rem (32 px — bbl.admin.ch's own hero `space-y-8` value) at the WIDEST tier only (≥1280); compact two-column keeps none (user decision, twice refined) |
| News list page | T: flat editorial list; S: card grid — neither matched the CD's own news page | **D46**: both = CD `newsList.vue` anatomy — tinted band (h1 + filter fields + removable chips), count + sort + list/gallery toggle (list default, thumbnails where images exist), pagination. Filters stay VISIBLE (user decision; CD collapses them behind a toggle) |
| News detail page | order diverged from CD (image before lead, eyebrows) | **D46**: both = CD `detailPressRelease.vue` order — meta (source \| date) → title → LEAD → figure → body |
| Search results media | both text-only rows/cards | **D47**: thumbnails where the record has an image (CD SearchResultsList shows media in both display types) — news, applications, buildings |
| Downloads sidebar (T) | filter sidebar open by default | **D48**: hidden by default (`sb=1` opts in) — user decision |
| Top bar: eGate | S: `top-bar-navigation` with eGate + External icon; T: none | **D49**: same link in T (≥1024 in the bar, in the burger drawer below — S's documented pattern) |
| Federal lockup gaps | T: extra 8 px flag→wordmark flex gap (the ~11 px optical gap already sits INSIDE the CD name SVG), sub-line +2 px, divider pinned 48 px + centred <1024, 78 px band floor | **D50**: CD values in T — gap 0, no sub-line offset, divider 40/56/70 ramp, no band floor; measured equal to S at 400/800/1280/1440 |
| Home news band | T: «Aktuell» carousel (CD carousel arrows/bullets, `card--profile` teasers); S: static 3-card «News» grid | **D51**: T's band ported to S byte-for-byte (heading «Aktuell», paging, dots, «Weitere News») |
| View toggles | three active styles (T catalogue: tint only; S catalogue: tint+ink+rail; new results toggles: color-only) | **D52**: ONE pressed style everywhere — tint secondary-100 + ink glyph + 3 px bottom rail (+ forced-colors outline); results toggles reuse `.view-switch__btn`; links mark active via class + `aria-current`, buttons via `aria-pressed` |
| Link underlines | CD: `main a` underlined AT REST (link.postcss:5-10), components opt out; both apps had hover-only pockets, T underlined chrome on hover | **D53**: content links underline at rest in both (greeting strip, contact block); chrome links (meta/top bar) hover = colour only in both |
| Home hero band padding | both used the BASE hero recipe (pt 48/56/80 + pb 56/80/128) | **D54**: CD overrides the `--main-image` variant to symmetric `container--py` (hero.postcss:73-74) = 56/80 @1024/128 @1920 — the 5rem@lg bbl.admin.ch renders live (user finding); both home heroes now symmetric |
| Hero content width (T) | hero content ran up to 64 px wider than every other band ≥~1670 px (`hero__inner` max-width didn't subtract the container padding) | **D55**: inner = the `.container` CONTENT box (`calc(max-width − 2×padding)`); edges measured flush with header/nav at 1000/1440/2000 |
| Top-bar internals | T: authorities icon 20 px fixed, 16 px flex gaps; S: eGate icon 16 px and 2 px above the bar centre | **D56**: External icons on the CD `icon--md` ramp (20 <768 / 24) in both links, both bars; chip mr-2 (8 px) + flush full-height cells (CD `h-full` chain); measured equal — all item midlines 24, gaps 8/0 |
| News in the navigation | T: a row inside the «Wissen und Hilfsmittel» drawer; S: own L1 entry | **D58**: L1 entry in both, same slot (… Wissen und Hilfsmittel · News · Meine Vorgänge), own active rail |
| Top-bar btn gap · meta details | T: authorities label→icon 4 px, name ink gray-500, divider = gray-300 background fill; S/CD: 8 px (icon ml-2), name text-600, separator = secondary-400 border stroke | **D59**: CD/service values in T — gap 8, name gray-600, divider 24 px secondary-400 border (24 \| rule \| 24 slots measured equal) |
| Header focus search (T) | open form sat 4.5 px below the toggle (icon jumped); the focused field's DS z-10 lift painted OVER the submit (magnifier vanished); native clear ✕ suppressed; input pr 40 < the 48 px submit overlay | **D60**: form vertically centred (service `.search__form` recipe) — icon dy 0 measured; submit z-lifted above the focused field; native type=search clear ✕ restored (service behaviour); pr 48 |
| Footer «Weitere Informationen» | T: three external bbl.admin.ch links; S: portal destinations in two sub-columns | **D61**: shared concept — portal destinations lead (T adds **Dienstleistungen** `#/services` and **Liegenschaften** `#/properties`, user request), external extras in the second sub-column; anatomy (rows, rules, right-hand glyphs) was already equal |
...
| D41 | Home quick-services band = the shared WHITE quick card (`card--quick` + corner `arrow-btn` + `.card-grid` auto-fit). Verified against CD source: **no grey icon tile exists** — `.box` is a stacked commerce box (box.postcss:5-12, OrderBox/ShoppingCart) and `bg-secondary-50` otherwise appears only on hovers/bands/placeholders; the CD teaser is the white `card--default`/`card--clickable` (shadow-lg → 2xl, title → primary-700). Section CTA = the CD's own `.section__action` + `btn--bare btn--icon-right` (section.postcss:89-93). Band heading = **«Häufig genutzte Dienstleistungen»** in both — the noun the nav and the CTA already use (was «Häufig genutzte Dienste» / «Häufig gebraucht») | CTA anatomy adopted (the bold primary link + 4 px hover slide retired); desc ink → gray-600; `card-grid` gains the sub-320 px `min()` guard; CTA verb «anzeigen»; heading via `home.services` (all four languages on the nav noun) | tiles converted from the grey icon tile to the shared card (icons stay in catalogue/drawers; `quick-tile` remains the my-cases bookmark tile on tinted bands); heading in `pages/home.js` |
| D42 | Prototype banner wrapper: 16 px column gap between copy and action, CD's `lg:ml-6` (24 px) from 1024, and the SAME text emphasis — the whole lead sentence bold («**Diese Anwendung ist ein Prototyp.**») | already so (reference) | gap + lg margin added (button previously sat flush under the text); emphasis widened from the single word «Prototyp» |
| D43 | Profile page in both portals behind the header name (identity + notifications + language cards, logout at the foot), content in ONE centred narrow column; NO role rows (user decision — «Aktive Rolle»/«Weitere Rollen» deleted: demo machinery, not identity; role switching stays programmatic) | role rows deleted from `#/profile`; identity list = Name / E-Mail / eIAM-Subjekt-ID / Verwaltungseinheit | new `js/pages/profile.js` (AGOV/FedLogin wording, `option-list` recipe), routed + meta-bar name becomes the link; content centred in `container__center--xs` like the tenant's `container--reading` |
| D44 | `greeting-strip` on the home page: time-of-day greeting, first name, open-case count as an inline link (+ Rückfrage count where present), quiet gray-50 field | already so (reference) | ported above the case-preview table (`pages/home.js` + `components/content.css`, tenant values) |
| D45 | `hero__title` top margin **2rem (32 px)** at the WIDEST tier only (≥1280) — the value bbl.admin.ch's live hero uses (Tailwind `space-y-8`, i.e. `calc(2rem*var(--tw-space-y-reverse))` margins); the compact two-column hero keeps none (user decision, twice refined: 24 px was still too small) | `sections/home.css` split-margin rule (`--space-xl`) | `layouts/shell.css` — md margin removed, ≥1280 `--sp-8` |
| D46 | News pages = the CD's own anatomy, verified in source. List (`newsList.vue` + search.postcss §SEARCH RESULTS): tinted band with h1 + filter fields (keyword, publisher/type, date range) + removable chips; results header count + bare sort + icon-only list/gallery toggle; LIST default with right-hand 180×120 thumbnails where images exist (hidden <480); pagination. Detail (`detailPressRelease.vue`): meta (source \| date) → title → lead → figure → body in the centred reading column. **Kept-deviation (user decision): the filter drawer stays VISIBLE — CD's «Filter anzeigen» collapse toggle is deliberately not used** | `renderNewsList` rewritten on the shared classes (rows/cards = the search renderers); detail reordered (lead before image, eyebrow retired); breadcrumb «News» | `pages/news.js` rewritten the same way (rows gain the media slot; gallery = the standard card); detail order confirmed |
| D47 | Search results carry the record's image where one exists — CD SearchResultsList shows thumbnails in both display types (`Card type=list/universal`) | news/applications/buildings rows + gallery cards get the shared 180×120 `object-cover` slot | same (`pages/search.js` index rows + card photos) |
| D48 | Tenant downloads page: the sidebar filter starts HIDDEN (`sb=1` opts in; `#/properties` keeps its open-by-default sidebar — different job: spatial browsing vs. filtering a flat list) | `docState.sidebar` default false | n/a (no sidebar downloads surface) |
| D49 | eGate in the navy top bar (CD `top-bar-navigation`, label + External icon, colour-only hover), hidden <1024 with the burger drawer carrying the link there — the service-portal's documented deviation from CD's icon-only mobile bar, now shared | new `top-bar-navigation` markup (shell.js) + styles; drawer link; `top.federalOffers` i18n | already so (reference — `TOP_BAR_LINKS`) |
| D50 | Federal lockup pixel-exact per `logo.postcss`/`top-header.postcss`: flag 30×33→32×34 @lg→40×44 @3xl; **no CSS gap flag→wordmark** (the ~11 px optical gap is inside the name SVG — first glyph at x≈15.3/244); separator ±8/16/24/32 margins and 40/56/70 heights; title −0.16 rem correction at every width; band padding 12/16/24/32/40 with no min-height floor | bundmark flex gap 8→0; sub-line 2 px offset removed; compact-tier divider 48 px + `align-self:center` and the 78 px band floor removed; measured equal to S at 400/800/1280/1440 | already CD (reference) |
| D51 | Home news band = «Aktuell» carousel (swisstopo pattern on CD carousel anatomy: 48/56 px arrows per carousel.postcss:74-76, 16 px bullet dots per :36-47 with 44 px hit slop, three `card--profile` teasers per page, «Weitere News» in the footer) | already so (reference) | static «News» card grid replaced; `card--profile` + `.news-section` CSS ported with token mapping (text-h3→text-xl etc., shadow-card-hover ≡ shadow-2xl); paging delegated on the page mount |
| D52 | ONE view-toggle pressed style, both portals, every surface (catalogue bars + news/search results headers): resting muted icon-only square ≥44 px, hover primary, active = secondary-100 tint + ink glyph + 3 px bottom rail (`#6B7280` — non-colour cue, WCAG 1.4.1) + forced-colors outline. Buttons mark active via `aria-pressed`, LINK toggles via `--active` class + `aria-current` (aria-pressed is invalid on links). CD defines the anatomy (two bare icon-only Btns, newsList.vue:167-182) but no pressed style — the portals' shared recipe fills that gap | catalogue `--active` gains the rail + forced-colors; news/search view links re-based on `view-switch__btn`; bespoke `search-results__view` rules deleted | pressed selector widened to `--active`; news toggle re-based on `view-switch__btn` (drawer-divider container replaces the separator span); `search-results__views` <640 hide shared |
| D53 | Link rule per CD `link.postcss:5-10`: content links in main underlined AT REST (offset 2), components opt out (`no-underline` — cards, buttons, tags, download items…); chrome links (top bar, meta navigation) never underline — hover is colour-only | greeting-strip + contact-block links → rest underline; meta-bar hover underline removed (colour-only) | greeting-strip link → rest underline (the foundations rule already covers content links) |

| D54 | Home hero (`--main-image`/split variant) vertical padding = CD `container--py` (hero.postcss:73-74): **symmetric 56 / 80 @1024 / 128 @1920** — 5rem at lg exactly as bbl.admin.ch renders; overrides the base hero's asymmetric 48/56/80 top, which stays correct for the other hero variants (user finding, CD-verified) | `.hero--split { padding-top: var(--section-py) }` | `.hero--main-image { padding-top: var(--section-py) }`; `test-hero-layout` D45 expectation updated (compact two-column = margin 0) |
| D55 | Hero content box ≡ the `.container` content box at every width — title and image flush with header/nav/section edges (measured 1000/1440/2000) | `.hero__inner` max-width now subtracts `2×container-padding` (it is padding-less; the raw max-width made the hero up to 64 px wider per side above ~1670 px) | already so (hero composed inside `.container`) |
| D56 | Top-bar internals byte-equal: External glyph boxes on the CD `icon--md` ramp (icons.postcss:27-29, 20 <768 / 24 from 768) in BOTH bar links; chip → nav gap = the CD badge `mr-2` (8 px, top-bar.postcss:163); nav and language switcher are flush full-height cells (CD `h-full` chain) whose own paddings carry the rhythm. Measured after: every item midline 24 in the 48 px bar, gaps 8/0, both apps | authorities svg 20→`icon--base`(24) @768; actions gap 16→0 with chip mr 8 | eGate icon `icon--base`→`icon--md`; nav `align-self:stretch` + full-height ul (glyph was 2 px high) |
| D57 | ONE browser-tab-title strategy: «\<kurzer Seitenname\> · BBL \<Portal\>» — a short curated page name (the breadcrumb label / the page's declared title), middle-dot separator, product last; home names itself «Startseite» in both. Verified live on ten routes («News · BBL Mieterportal» ↔ «News · BBL Kundenportal», «Startseite · …» both) | title source h1 → breadcrumb label (the h1 put whole hero sentences into the tab); separator — → ·; home «Startseite»; `bc.profile` «Mein Profil» for crumb/tab parity | already so (reference — router `setTitle`) |
| D58 | News is a TOP-LEVEL nav entry in both portals, same slot: … Wissen und Hilfsmittel · **News** · Meine Vorgänge; the news pages carry their own active rail. The tenant row now holds six L1 entries against the CD's «limit to five» guidance — accepted (user decision): the shared navigation concept outweighs the count, and News buried in a drawer was unreachable from the chrome | News row removed from the «Wissen» drawer (`infoMenu`); `newsNavItem()` added to every role's nav; news list/detail `activeNav: 'info'` → `'news'` (the info overview page keeps its News cross-link card) | already so (reference) |
| D59 | Top-bar/meta detail parity (user finding): authorities label→icon gap = the CD bar-icon `ml-2` (8 px, top-bar.postcss:141); signed-in name ink = the sister's text-600 (#4B5563); the name/Abmelden divider = the DS `.separator--vertical` STROKE recipe — 24 px border-right in secondary-400 (intranet blue-grey), not a gray-300 background fill. Slots re-measured equal after (24 \| rule \| 24 in both; a first attempt at margin arithmetic was reverted when measurement showed the sister's real slots) | gap 4→8; `.top-header__meta-user` gray-500→gray-600; divider rewritten on the border recipe | already so (reference) |
| D60 | Tenant focus search = the service/CD open-field anatomy (user finding): the magnifier REMAINS IN PLACE on expand and a clear ✕ appears while typing. Three defects fixed: (a) the form top-aligned instead of centring — icon jumped 4.5 px down (now `top:50% + translateY(-50%)`, the service `.search__form` recipe; dy 0 measured); (b) the DS focus lift (`:focus-visible { z-index:10 }`) painted the autofocused field OVER its own submit — the magnifier vanished entirely (submit now z-11); (c) the native type=search cancel button was suppressed and the input's 40 px right padding ran text under the 48 px submit overlay (suppression removed, pr = 48 like the service's sp-12). Mobile full-width row re-anchored (`transform:none`) | all three + mobile override | already so (reference) |
| D61 | Footer «Weitere Informationen» carries the SAME concept in both: portal destinations lead in the first sub-column (arrow glyph), external extras follow (External glyph). Tenant adds **Dienstleistungen** (`#/services`) and **Liegenschaften** (`#/properties`) per user request; Rechtliche Grundlagen / E-Rechnung / Kontakt move to the second sub-column. Row/rule/glyph anatomy was already byte-equal from D16 | links column restructured (i18n labels `nav.services`/`nav.properties`) | already so (reference) |
...
11. ~~News overview presentation~~ — superseded by **D46**: both portals now render the same CD news anatomy (list default + gallery toggle).
12. **News filter drawer always visible** (CD collapses it behind «Filter anzeigen», newsList.vue:60-88) — with four fields the indirection bought nothing; user decision, both apps.
13. **Results-page view switch hidden <640** (CD keeps its toggle at every width) — the list view works everywhere, the gallery is a preference; both apps.
14. **eGate bar entry hidden <1024, drawer copy instead** (CD keeps an icon-only bar copy below lg) — an unlabelled icon route beside the drawer's named one; service's decision, now shared.
15. **View-toggle pressed style** (tint + ink + 3 px rail) — the CD ships the bare-button anatomy but no pressed treatment; the shared recipe adds the WCAG 1.4.1 non-colour cue.
...
- **D41/D42 (2026-08-22):** `css/components/cards.css` — `.section-cta` replaced by the CD `.section__action`; `card--quick__desc` → gray-600; `card-grid` `min()` guard. `css/components/buttons.css` — DS `--icon-left/right` modifiers. `js/app.js` — both section CTAs on the CD `btn--bare btn--icon-right` anatomy. `data/i18n.json` — «Alle Dienstleistungen anzeigen». `css/foundations/print.css` — suppression selector follows the rename.
- **D43–D48 (2026-08-22):** `js/app.js` — profile role rows deleted (identity list only); `renderNewsList` rewritten on the CD anatomy (band + fields + chips + count/sort/toggle + shared result renderers + pagination); news detail reordered (meta → title → lead → figure); search rows/cards pass `image`; downloads `docState.sidebar` defaults hidden (`sb=1` opts in). `css/sections/secondary-pages.css` — `.search__filters` block. `css/sections/home.css` — hero-title margin ≥1280 only. `data/i18n.json` — `bc.news` «News».
- **D49–D53 (2026-08-22):** `js/shell.js` — `top-bar-navigation` (eGate) + drawer copy. `data/i18n.json` — `top.federalOffers`. `css/navigations/header.css` — top-bar-navigation styles; lockup: bundmark gap 0, sub-line offset 0, compact divider/band-floor overrides removed; meta hover = colour-only. `css/sections/properties.css` — unified view-switch pressed style (rail + forced-colors; `aria-pressed` alias). `css/sections/secondary-pages.css` — bespoke result-toggle rules deleted; drawer always-open form. `css/sections/home.css` + `css/sections/info.css` — greeting-strip/contact links underline at rest. `scripts/verify/check-search-improvements.mjs` — hero-margin assertion updated to the D45 contract.
- **D45 (final)/D54/D55 (2026-08-22):** `css/sections/home.css` — split hero title margin 2rem ≥1280 (bbl.admin.ch `space-y-8` value). `css/layouts/layout.css` — `.hero--split` symmetric `--section-py` padding; `.hero__inner` = container content box.
...
- **D41/D42 (2026-08-22):** `css/components/card.css` — shared `card--quick`/`arrow-btn`/`card-grid` block (class-portable with the tenant). `js/pages/home.js` — «Häufig gebraucht» tiles converted (icons dropped there; `quick-tile` stays for the my-cases bookmarks). `css/utilities.css` — dead white-section tint variant removed. `css/layouts/page.css` — banner wrapper 16 px gap. `css/components/feedback.css` — banner action `lg:ml-6` restored. `js/app.js` — banner lead sentence bold.
- **D43–D48 (2026-08-22):** new `js/pages/profile.js` (+ route, + test-routes entry); `js/ui/shell/header.js` — meta name → `#/profile` link; `css/navigations/header.css` — quiet name-link rule; `css/navigations/drawer.css` — name row is a control now; `css/components/form.css` — `option-list` recipe; `js/pages/home.js` — greeting strip; `css/components/content.css` — `.greeting-strip`; `css/layouts/shell.css` — hero-title margin ≥1280 only; `js/pages/news.js` — CD list/detail anatomy; `css/sections/search.css` — results chrome + media-row block; `js/pages/search.js` — result images; `css/components/card.css` — news-detail photo square.
- **D49–D53 (2026-08-22):** `js/pages/profile.js` — content centred (`container__center--xs`); `js/pages/news.js` — filters always visible (toggle machinery removed), view toggle on `view-switch__btn` links; `css/sections/search.css` — drawer always-open form, `search-results__views` <640 hide, bespoke toggle rules deleted; `css/components/content.css` — view-switch link-safe (`text-decoration:none`) + `--active` alias + «Aktuell» carousel + `card--profile` port + greeting-strip link underline; `css/layouts/grid.css` — forced-colors alias; `js/pages/home.js` — «News» grid → «Aktuell» carousel with delegated paging.
- **D45 (final)/D54 (2026-08-22):** `css/layouts/shell.css` — hero title margin 2rem ≥1280; `.hero--main-image` symmetric `--section-py` padding. `scripts/test-hero-layout.mjs` — compact-tier expectation updated to the D45 contract.
...
- **2026-08-22 evening batch (D43–D53):** tenant `npm test` + `check-header-chrome` 27/27 + `check-mobile-nav` 12/12 + `check-search-improvements` (one assertion updated to the D45 contract) + `check-a11y-responsive` (failureCount 0); service `test-routes`/`test-content`/`test-search`/`test-ui-state`/`test-login`/`check-banner` green. Lockups re-measured EQUAL at 400/800/1280/1440 (flag size, flag→wordmark 0, separator margins 8/16/24 and heights 40/56, title offset); view-toggle pressed style probed computed-identical in both apps (`rgb(223,228,233)` tint + `#6B7280` inset rail); carousel paging, eGate link, centred profile, always-open filters verified by screenshot.
...
- S's `.kv` definition lists append «&nbsp;:» to labels (portal-wide convention, visible on the profile identity card); T's lists carry bare labels.
- Brand-band height <480 differs ~2.5 px (right-cluster content: S carries the cart control) — lockup geometry itself is equal.
